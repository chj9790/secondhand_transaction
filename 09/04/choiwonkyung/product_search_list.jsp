...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadMidRegions(top) {
    ...
    if (detailSelect) detailSelect.innerHTML = '<option value="">-- 동/리 선택 --</option>';
...
<body>
  ...
  <input type="hidden" name="area" id="finalArea" value="${selectedArea }" />
  <div id="area-radios"> </div>
    <button type="button" id="btnUseMyLocation" style="margin-top:10px; padding:5px 10px;">우리 동네 보기</button>
	<button type="button" id="btnHideMyLocation" style="margin-top:10px; padding:5px 10px; display:none;">숨기기</button>
...
<script>
    ...
    // 현재 선택 지역
    let selectedArea = document.getElementById('finalArea').value || ''; 
    ...
    // 라디오 버튼 갱신
        ...
        inputAll.name = 'area';
        inputAll.value = '';
        inputAll.checked = !selectedArea; // 선택값 없으면 전체보기 체크
        inputAll.addEventListener('change', () => {
        selectedArea = '';
        document.getElementById('finalArea').value = '';
        triggerFilter();
        });
	 ...
	 // 동/리 리스트 라디오
        areaList.forEach(area => {
            ...
            input.addEventListener('change', () => {
                selectedArea = area;
                document.getElementById('finalArea').value = area; 
                triggerFilter();
            });
